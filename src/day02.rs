...
mod tests {
    ...
    #[test]
    fn test_intcode_1() {
        let memory = Intcode::new("1,9,10,3,2,3,11,0,99,30,40,50").run(HashMap::new());
        assert_eq!(memory.get(&0).unwrap_or(&0), &3500);
    }

    #[test]
    fn test_intcode_2() {
        let memory = Intcode::new("1,0,0,0,99").run(HashMap::new());
        assert_eq!(memory.get(&0).unwrap_or(&0), &2);
    }

    #[test]
    fn test_intcode_3() {
        let memory = Intcode::new("2,3,0,3,99").run(HashMap::new());
        assert_eq!(memory.get(&3).unwrap_or(&0), &6);
    }

    #[test]
    fn test_intcode_4() {
        let memory = Intcode::new("2,4,4,5,99,0").run(HashMap::new());
        assert_eq!(memory.get(&5).unwrap_or(&0), &9801);
    }

    #[test]
    fn test_intcode_5() {
        let memory = Intcode::new("1,1,1,4,99,5,6,0,99").run(HashMap::new());
